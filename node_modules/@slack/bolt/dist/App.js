...
class App {
    ...
    view(callbackId, ...listeners) {
        this.listeners.push([builtin_1.onlyViewSubmits, builtin_1.matchCallbackId(callbackId), ...listeners]);
    }
    error(errorHandler) {
    ...
    async onIncomingEvent({ body, ack, respond }) {
        ...
        const listenerArgs = {
            ...
            payload: (type === helpers_1.IncomingEventType.Event) ?
                bodyArg.event :
                (type === helpers_1.IncomingEventType.ViewSubmitAction) ?
                    bodyArg.view :
        ...
        else if (type === helpers_1.IncomingEventType.ViewSubmitAction) {
...
function buildSource(type, channelId, body) {
    ...
    const source = {
        teamId: ((type === helpers_1.IncomingEventType.Event || type === helpers_1.IncomingEventType.Command) ? body.team_id :
            (type === helpers_1.IncomingEventType.Action || type === helpers_1.IncomingEventType.Options || type === helpers_1.IncomingEventType.ViewSubmitAction) ? body.team.id :
        ...
        enterpriseId: ((type === helpers_1.IncomingEventType.Event || type === helpers_1.IncomingEventType.Command) ? body.enterprise_id :
            (type === helpers_1.IncomingEventType.Action || type === helpers_1.IncomingEventType.Options || type === helpers_1.IncomingEventType.ViewSubmitAction) ? body.team.enterprise_id :
                undefined),
                ...
                (typeof body.event.user === 'object') ? body.event.user.id :
                    (body.event.channel !== undefined && body.event.channel.creator !== undefined) ? body.event.channel.creator :
                        (body.event.subteam !== undefined && body.event.subteam.created_by !== undefined) ? body.event.subteam.created_by :
                            undefined) :
            (type === helpers_1.IncomingEventType.Action || type === helpers_1.IncomingEventType.Options || type === helpers_1.IncomingEventType.ViewSubmitAction) ? body.user.id :
